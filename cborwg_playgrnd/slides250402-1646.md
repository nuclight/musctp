footer:CBOR 2025-04-02 • Vadim Goncharov <vadimnuclight@gmail.com> * bad MD-to-PDF formatting by deckset.com
slidenumbers: true
theme: work, 1
autoscale: true
build-lists: true
...
[VG]
Notice here usage of `simple(N)` as an "insertion" of several items (one may say CBOR Sequence).
But `cbor-packed` do NOT support CBOR Sequences! Tables in cbor-packed can contain only complete CBOR items.
